...
def line_optimization(problem, number_of_points,  plot_obj=1, plot_const=0,plot_inp=1, sweep_index=0):
    ...
    if plot_inp==1:
        ...
        inp_range = list(range(0,free_inp_num))
...
def opt_setup(problem,input_0,solver='SNOPT',FD='single', sense_step=1.0E-6,  nonderivative_line_search=False,\
    ...
    import pyoptsparse
    opt_prob = pyoptsparse.Optimization('SUAVE',mywrap)
    ...
    if solver == 'SNOPT':
        import pyoptsparse.pySNOPT
        opt = pyoptsparse.pySNOPT.SNOPT()
        ...
        import pyoptsparse.pyCOBYLA
        opt = pyoptsparse.pyCOBYLA.COBYLA()

    elif solver == 'SLSQP':
        import pyoptsparse.pySLSQP
        opt = pyoptsparse.pySLSQP.SLSQP()
        opt.setOption('MAXIT', 200)
    elif solver == 'KSOPT':
        import pyoptsparse.pyKSOPT
        opt = pyoptsparse.pyKSOPT.KSOPT()
    elif solver == 'ALHSO':
        import pyoptsparse.pyALHSO
        opt = pyoptsparse.pyALHSO.ALHSO()
    elif solver == 'FSQP':
        import pyoptsparse.pyFSQP
        opt = pyoptsparse.pyFSQP.FSQP()
    elif solver == 'PSQP':
        import pyoptsparse.pyPSQP
        opt = pyoptsparse.pyPSQP.PSQP()
    elif solver == 'NLPQL':
        import pyoptsparse.pyNLPQL
        opt = pyoptsparse.pyNLPQL.NLPQL()
    elif solver == 'NSGA2':
        import pyoptsparse.pyNSGA2
        opt = pyoptsparse.pyNSGA2.NSGA2(pll_type='POA')
    elif solver == 'MIDACO':
        import pyoptsparse.pyMIDACO
        opt = pyoptsparse.pyMIDACO.MIDACO(pll_type='POA')
    elif solver == 'ALPSO':
        import pyoptsparse.pyALPSO
        #opt = pyoptsparse.pyALPSO.ALPSO(pll_type='DPM') #this requires DPM, which is a parallel implementation
        opt = pyoptsparse.pyALPSO.ALPSO()
