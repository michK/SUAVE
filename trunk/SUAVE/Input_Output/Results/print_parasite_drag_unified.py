...
# ----------------------------------------------------------------------
## @ingroup Input_Output-Results
def print_parasite_drag_unified(ref_condition,vehicle,analyses,filename = 'parasite_drag.dat'):
    """This creates a file showing a breakdown of compressibility drag for the vehicle. Estimates
    ...
    for propulsor in vehicle.propulsors:
        drag_breakdown[propulsor.tag].tag = propulsor.tag[0:25]
        CD_p     += drag_breakdown[propulsor.tag].parasite_drag_coefficient
        drag_breakdown[propulsor.tag].parasite_drag_coefficient = drag_breakdown[propulsor.tag].parasite_drag_coefficient
        swet_tot += drag_breakdown[propulsor.tag].wetted_area

    drag_breakdown['pylon'].tag = 'pylons'
    ...
    for k in drag_breakdown:
        if isinstance(k,Data):
            ...
            wetted_area     =   str('%11.3f'   % k.wetted_area)                 + '    |'
    ...
    # String formatting for miscellaneous drag
    ...
    swet_tot     =  str('%11.3f'   % swet_tot)                 + '    |'
    CD_p         =  str('%11.5f'   % CD_p    )                 + '    |'
    fid.write(38*' ' + '|'+ 6*( 15*' '+'|') + '\n')
    fid.write(28*' ' + 'SUM       |' + CD_p + swet_tot + 4*( 15*' '+'|') + '\n')
